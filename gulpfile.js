...
const
	...
	themeTextDomain = 'aquago', // run `gulp textdomain` separately for replacing domain

	isGutenberg = true, // whether using Gutenberg in theme or not
...
function textdomain() {
	...
	return gulp.src('./**/*', {ignore: ['gulpfile.js', dir.nm]})
		.pipe(replace('aquago', themeTextDomain))
		.pipe(gulp.dest('./'));
